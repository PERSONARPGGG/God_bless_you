...
## [v2.6] — 2026-07-06 · 리브랜딩 "밤의 목장" + 멘토 피드백 UI

### 변경
- **앱 이름 확정: "밤의 목장"** (부제 "오늘 밤, 주님이 누이시는 곳", 시편 23:2) — 팀 논의 + 목사님 에이전트 검토 거침. "오늘은 어땠나요?"는 메인 화면 인사말로 유지
- 타이틀·manifest·설정 앱 정보·공유 문구·환영 화면·내보내기 헤더 전면 교체 (ko: 밤의 목장 / en: Night Pasture / ja: 夜の牧場)
- 감사 한 줄 일기 "저장하기" 버튼 제거 → "오늘 묵상 마치기"(또는 5분 무활동) 시 자동 저장
- "오늘 묵상 마치기" 버튼 sticky — 스크롤 어디서든 화면 하단 표시, PWA 설치 배너와 겹침 방지
- 클로징 후 버튼 라벨 "다시 시작하기" → "처음으로"
- 보관함 탭 재구성: [감정 · 통계 (통합, 기본)] / [즐겨찾기] 2탭

### 추가
- 달빛 아래 양 한 마리 — 화면 하단을 배회하다 강아지처럼 앉는 CSS/JS 장식 (prefers-reduced-motion 시 숨김)
- 찬송가 음원 5곡 추가 (총 10곡)

## [v2.5] — 2026-07-02 · 클로징 + 감성 연출 + 프리미엄 경계 + 저작권 정리

### 추가
- 결과화면 클로징(마무리) 트리거 3종: 일기 저장 / "오늘 묵상 마치기" 버튼 / 5분 무활동 타이머 → 마무리 카드 + 스트릭 연출, 마무리 후 보관함 동선
- 감성 연출: 별똥별(랜덤+클로징 1회), 클로징 시 밤하늘 밝아짐, 감정색 물들임(선택 감정 색이 이후 스텝에 스며듦), 스텝 전환 밤하늘 흐름
- 연속 감정 인식: 같은 결 감정 2일+ 이어지면 응원 카드에 인식 문장 (AI 미사용, moodHistory 규칙 기반)
- 프리미엄 경계 구조: 대시보드 잠금 카드 + 혜택/가격(₩1,900/월) 모달 + "출시 알림 받기" 수요 검증
- 말씀 카드 이미지 공유: 밤하늘 감성 캔버스(1080×1350) 생성, Web Share 파일 공유 + 다운로드 폴백
- 배경 음악을 Web Audio 찬송가 오르골로 교체 (나 같은 죄인 살리신·예수 사랑하심은, PD 멜로디 자체 연주 — 저작권 클리어, mp3 불필요)
- 페르소나 데모 라벨 이야기형 이름으로 교체 (직장인 준호/워킹맘 수진/주부 은혜/김요한 목사/유학생 안나)

### 수정
- 마무리 카드 텍스트 흰색 → 웜다크 (밝은 카드 배경 가독성)
- 결과 카드 순서: 말씀→이야기→응원→일기 (일기 저장 클로징이 묵상 카드를 읽기 전에 숨기던 문제)
- 결과 카드 등장 애니메이션 이중 시스템(card-reveal+card-cascade) → cascade 단일화
- 클로징 타이머 모바일(<600px)에서 상단 아이콘과 겹침 수정
- 마무리 카드 제목·감정별 문구, 배경 음악 상태 텍스트 다국어 누락 수정 (ko/en/ja 키 150개 패리티 검증)
- 설정 저작권 고지를 ESV 요구 형식 전문으로 교체
